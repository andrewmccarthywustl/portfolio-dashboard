...
class StockPortfolioApp:
            ...
            try:
                ...
                for _, row in df.iterrows():
                    self.tree.insert("", "end", values=(
                        row['Date'], row['Symbol'], f"{row['Quantity']:.4f}",
                        f"{row['Buy Price']:.2f}", f"{row['Current Price']:.2f}", row['Last Updated']))
            except Exception as e:
                messagebox.showerror("File Error", f"Error loading transactions: {str(e)}")
        else:
            print(f"Transaction file {CSV_FILE} not found. Starting with an empty portfolio.")

    def fetch_stock_data(self, symbol, last_updated):
    ...
    def update_single_entry(self, item):
        ...
        new_price, new_last_updated = self.fetch_stock_data(symbol, last_updated)
